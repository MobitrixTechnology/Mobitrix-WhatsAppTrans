...
int androidpareser::towork( const char* androidpath)
{
	...
	if(readmessageinffromandroid()==-1)
	{
		readmessageinffromandroidEx();
	}
...
int androidpareser::readmessageinffromandroid()
{
	...
	char slqstr[1024];
	int sqllen = sprintf(slqstr, "select _id,chat_row_id,from_me,key_id,status,text_data,timestamp,message_type,starred,sender_jid_row_id,received_timestamp from message");
	slqstr[sqllen] = 0;
	...
	if (osqliteparser.SqlQuery(slqstr, vmapstr))
	{
		...
		//	printf("androidpareser", "code:10 itype:%d", itype);
			...
			{
				if(isgroup)
					groupmemberjid = FindDataFromMap(vmapstr.at(n), "sender_jid_row_id");
				else
				{
...
int androidpareser::readmessageinffromandroidEx()
{
	mselfjid = "me";
	printf("Read chat information, please wait\n");
	int msize = 0;
	tsqliteparsing osqliteparser(mmsgstoredbfile.c_str());
	char slqstr[1024];
	int sqllen = sprintf(slqstr, "select _id,key_remote_jid,key_from_me,key_id,status,data,timestamp,media_wa_type,remote_resource,received_timestamp,send_timestamp,media_url,media_mime_type,media_name,media_size,media_caption from messages");
	slqstr[sqllen] = 0;
	std::vector<std::map<std::string, std::string> > vmapstr;
